// server/engine/polish.js —— 全书打磨（借鉴 show-me-the-story 工单化 polish）
...
import { isCompletedChapter, hasExplicitCompletedStatus, transitionChapterStatus } from './chapter_status.js'; // V0.93.2：状态写入单一真源
...
export async function runPolish(bookId, opts = {}) {
  ...
  if (pendingRewrites.size > 0) {
    ...
    try {
      const { prepareAndCommitNarrativeRevision } = await import('./narrative_state.js');
