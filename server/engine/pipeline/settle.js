// server/engine/pipeline/settle.js —— 章结算：抽取事实/角色状态/时间线/伏笔动作/摘要/新设定
'use strict';
...
import { applyFacts, characterNames } from '../narrative/factbook.js';
import { applyForeshadowActions, allForeshadowsText } from '../narrative/foreshadow.js';
import { applyDeathAndCardEnrich } from '../narrative/characters.js'; // V0.37：死亡自动退场 + 卡片丰富
import { tidyPendingEntities } from '../narrative/pending.js'; // V0.40：待登记实体自动治理
import { compressProtagonistState } from '../planning/growth.js'; // V0.75 主角 state_json 压缩（防过时叙事键堆积）
import { touchEntities } from '../planning/world_expansion.js'; // V0.76 记录地点/势力登场章节（世界展开检测数据源）
import { extractEraMarkers } from '../narrative/history.js'; // V0.82 历史纪年抽取（公元年/年号纪年/季节 → timeline 锚点）
import { isSaneName, entityNamePlausible } from '../narrative/names.js'; // V0.83 角色名质量门（占位名/称谓名不建卡） // V0.93.11 实体类型名合理性门（物品名/短语名不冒充地点/势力）
import { historicalChapterFrame, syncHistoricalProtagonistState } from '../longform/historical_state.js';
import { reconcileStoryArcsForChapter, touchCharacters, registerOutlineLocations, normalizeCharacterAnchors } from '../quality/pleasure.js'; // V0.93.5 台账联动 // V0.93.7 last 补平
import { transitionChapterStatus } from './chapter_status.js'; // V0.93.2：状态写入单一真源
import { applySettlementMemories } from '../narrative/narrative_memory.js'; // V0.95：叙事记忆提取落库（人物声音/承诺/道具细节）
import { appendRollingRecent, rollingText } from '../narrative/rolling.js'; // V0.95：滚动摘要两段式（settle 只写 recent 段）
import { touchItems } from '../narrative/items.js'; // V0.95：物品/势力锚点推进
import { ensureVolumeSummaries } from './archive.js'; // V0.95：卷完成懒生成卷摘要（卷速查表）
import { sanitizeStoryMemoryText, detectEditorialVoiceLeak } from '../quality/rules.js';
import {
  assertNarrativeStateReady, recordSettledNarrativeExtension, validateNarrativeProjection,
  deterministicChapterProjection,
} from '../narrative/narrative_state.js';
import { recordChapterDiversityFeatures } from '../quality/chapter_diversity.js';
