// server/engine/write.js —— 场景正文生成（缓存感知：正文进历史堆；固定注入区 + 长度自愈）
'use strict';
...
import { characterRollCallText, characterCardsText, growthStatus, longRunningStateDebtText, rivalCharactersInScene } from './characters.js'; // V0.37：角色点名册（活跃/退场）；V0.49：角色卡关键字段；V0.74 题材感知成长状态 // V0.108：对手型角色出场检测（人物活性注入）
import { activateEntriesText } from './worldbook.js';
...
import { activeForeshadowsText, approachingForeshadowsText, overdueForeshadowsText } from './foreshadow.js';
import { relevantFacts, formatFacts, relevantFactsSmart } from './factbook.js'; // V0.83 语义增强事实召回
import { locationCardText } from './locations.js'; // V0.71：地点卡注入（稳定地点简短、变化地点强调）
import { buildPleasureContext } from './pleasure.js';
import { rollingText } from './rolling.js'; // V0.95：滚动摘要两段式统一读取
import { narrativeMemoryText } from './narrative_memory.js'; // V0.95：叙事记忆注入（人物声音/道具细节）
import { itemCardsText } from './items.js'; // V0.95：物品/势力场景卡注入
import { ensureHistory } from './outline.js';
import { PLOT_DEAI_TEXT } from './plot_ai.js'; // V0.80 剧情发展去AI味（硬要求注入）
import { poetryForScene, eraContextText, eraBoundaryText, currentStoryYear, isHistoricalEraBook, eraRedLineCheck } from './history.js'; // V0.81 诗词融入 + 时代红线节选 // V0.82 史实边界注入 // V0.88 宋末书锚点判定 // V0.105.7 时代器物写侧硬闸
import { HISTORY_DEAI_TEXT, WARFARE_ANCHORS, COURT_ANCHORS } from '../data/history.js'; // V0.81 历史去AI味 // V0.87 宋蒙战争考据锚点 // V0.88 南宋朝堂考据锚点
import { historicalPhaseForVolume, historicalScaleRegisterText } from './historical_longform.js'; // V0.91 四十年章节坐标 // V0.95.8 山河尺度分层纪律
import { crossYearOpeningIssue } from './historical_guardrails.js'; // V0.95.7 跨年开篇写审同源核查
import { isCompletedChapter, transitionChapterStatus } from './chapter_status.js'; // V0.93.1：完成态单一真源 // V0.93.2：状态写入单一真源
import { sanitizeStoryMemoryText } from './rules.js'; // V0.97.2：故事记忆不得夹带编辑话语
import { openingReaderContractText } from './opening_intervention.js'; // V0.98：只注入读者结构契约，绝不注入前置正文
...
import { buildCraftQuotaText } from './craft_quota.js';
import { compileBookDiversityContract } from './chapter_diversity.js';
import { compileBookCraftOccupancy, healCraftMorphology, buildBookLedgerBrief } from './craft_occupancy.js';
import { closeTrailingSentence } from './polish.js'; // V0.105.5：末句完整性收口（autoHealSceneLength 共用入口）
