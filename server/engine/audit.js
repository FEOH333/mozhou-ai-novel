// server/engine/audit.js —— 一致性审校（本地规则 + LLM 审校 + 细纲覆盖校验 + 修订）
'use strict';
...
import { historicalContinuityIssues, historicalYouthAuthorityTextIssues, historicalFigureTimelineIssues, phaseCoveredByText, stampOpeningTimelineProseFix, eraAnchorIssues } from './historical_guardrails.js'; // V0.93：确定性历史连续性/少年权限硬防线 // V0.93.10：历史人物登场窗 // V0.102.4：开篇时间线不得整章重规划 // V0.105.7：章首年号锚点
import { historicalFiguresFor, eraRedLineCheck } from './history.js'; // V0.93.10：历史人物档案读取（DB 优先+内置种子） // V0.105.7：时代器物审侧确定性扫描
import { historicalPhaseForVolume, historicalScaleRegisterText } from './historical_longform.js'; // V0.95.8 山河尺度纪律（写审同源一把尺）
import { isCompletedChapter } from './chapter_status.js';
import { applyValidatedSceneRewrite } from './polish.js';
import { openingReaderContractText } from './opening_intervention.js';
import { appendPublicationFeedback } from './publication_feedback.js'; // V0.99：写审同源的推流反馈 L4
import { getGlobal } from '../config.js'; // V0.108：开篇期阈值与全局配置同源（openingBlueprintChapters）
import { resolveCraftProfile, evidenceCertaintyThreshold } from './craft_profile.js';
import { compileBookDiversityContract, diversityRegression } from './chapter_diversity.js';
import { compileBookCraftOccupancy, healCraftMorphology, craftRegression } from './craft_occupancy.js';
...
export async function auditChapter(bookId, chapterId, { signal, streamCb } = {}) {
  ...
  if (book.genre === '历史') {
    try {
      const { eraContextText, eraBoundaryText } = await import('./history.js');
