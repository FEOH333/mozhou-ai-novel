// server/engine/pilot.js —— AI 本位：一键自动全书（从当前进度连续写多章，checkpoint 可中断续跑）
...
import { ensureHistory, generateBookContract, generateBookOutline, generateVolumeOutline,
  generateChapterOutline, volumeChapterCount } from './outline.js';
import { generateBookSettings } from './settings.js';
import { planBookPleasure } from './pleasure.js';
import { runPolish, smoothTransitions, shouldRunMidStoryReview, readMidStoryCursor } from './polish.js';
import { logFlow } from '../util/oplog.js';
import { getGlobal } from '../config.js';
import { autoRecover, detectDrift, recoveryConfig, recordChapterHealth } from './recovery.js';
import { autoReviewVolumes, runVolumeReview, reviewDueVolumes, shouldReviewVolumeBeforeLazyOutline, volumeReviewDisplayEvent } from './volumereview.js'; // V0.41 卷体检；V0.67 补体检候选
...
import { tidyRoster, sweepVolumeCastDesign } from './roster.js'; // V0.70：动态 import 静态化（无循环依赖，静态更清晰） // V0.105.4 cast 设计持久化 sweep（runCastDesign 移入 sweep 内部）
import { tidyFacts } from './factbook.js'; // V0.68 事实库整理
import {
...
} from './alignment.js'; // V0.45：三层大纲对齐（章名/卷名/卷大纲/书纲自动修订）
import { recoverMissingChapterContent } from './data_safety.js'; // V0.91：字数在但正文空时从最佳快照只增恢复
import { persistLifecycleCheckpoint } from './longform_lifecycle.js'; // V0.92：每卷阶段/兑付债务断点
...
import { ensureStoryPromiseProfile } from './story_promise.js'; // V0.98：先建立作品独有承诺，再规划开篇
import {
  prepareOpeningBeforeChapterOne, applyNewBookOpeningCandidate, maybeReviewOpening,
} from './opening_intervention.js';
import {
  shouldRefreshPublication, syncFanqiePublication,
} from './publication_feedback.js'; // V0.99：自动刷新发布边界并持续提示推荐失败
...
export async function runBookPilot(bookId, opts = {}) {
  ...
  try {
    const { ensureEraContext } = await import('./history.js');
  ...
  try {
    const { generateOpeningBlueprint } = await import('./opening.js');
 ...
 // cast_design（实测 49 章实证 vol1-5 连烧 5 次、cast_text 被重写 5 次）。
  ...
  while (true) {
    ...
    try {
      ...
      try {
        const { checkPromiseFulfillment } = await import('./promise.js');
      ...
      try {
        const { signingDue, simulateSigningReview, rewriteOpening } = await import('./signing.js');
    ...
    if (written > 0 && written % 5 === 0 && !signal?.aborted) {
      ...
      try {
        const { tidyLocations } = await import('./locations.js');
    ...
    if (!signal?.aborted) {
      try {
        ...
        if (shouldRunMidStoryReview({
          ...
          const { midStoryReview } = await import('./polish.js');
    ...
    try {
      ...
      if (!ca.aligned && ca.autoFix) {
        const { isPublishedChapterIdx } = await import('./alignment.js');
    ...
    try {
      ...
      if (vol) {
        ...
        if (volDone && (!prevReview || prevReview.status === 'failed')) {
          ...
          try {
            const { foreshadowClosurePlan } = await import('./foreshadow.js');
