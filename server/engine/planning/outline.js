// server/engine/planning/outline.js —— 大纲生成（书契约/书级/卷级/章细纲 + 写前五问自检）
'use strict';
...
import { activeForeshadowsText, approachingForeshadowsText } from '../narrative/foreshadow.js';
import { rollingText } from '../narrative/rolling.js'; // V0.95：滚动摘要两段式统一读取
import { sanitizeStoryMemoryText, titleShapeStreakIssues, titleRootRepeatIssues } from '../quality/rules.js'; // V0.107：卷纲期章名句式族/近词根校验
...
import { narrativePatternSignature, repeatedPatternIssue } from '../quality/narrative_patterns.js';
import { narrativeLessonsText } from '../quality/narrative_lessons.js';
import { compileBookDiversityContract, diversityContractIssues } from '../quality/chapter_diversity.js';
import {
...
import { forgottenList } from '../narrative/foreshadow.js';
import { relevantFacts, formatFacts, relevantFactsSmart } from '../narrative/factbook.js'; // V0.83 语义增强事实召回
import { buildPleasureContext, formatPleasurePlan } from '../quality/pleasure.js'; // V0.80 快感计划贯通
import { generateIdeaSeeds, scoreContract, generateBookTitle } from './idea.js';
import { characterRollCallText, growthStatus, longRunningStateDebtText } from '../narrative/characters.js'; // V0.37：角色点名册（活跃/退场）；V0.74 题材感知成长状态
...
import { historyAnchorsText, eraContextText, historyGrowthNote, poetryForScene, eraBoundaryText, currentStoryYear, historyNamingRules, isHistoricalEraBook } from '../narrative/history.js'; // V0.81 史实锚定 + 时代背景卡 + 成长波动 + 诗词 // V0.82 史实边界 + 取名规则 // V0.88 宋末书锚点判定
...
import { historicalPhaseForVolume, historicalPhaseText, historicalLongformPlanText, normalizeHistoricalBookOutline, mergeHistoricalChapterFrame, validateHistoricalVolumeOutline, historicalScaleBeatRule } from '../longform/historical_longform.js'; // V0.91 四十年阶段/年代硬校验 // V0.95.8 山河尺度节拍（细纲层落实远景/全景配额）
import { historicalOutlineIssues, historicalYouthAuthorityIssues, healHistoricalCheckpointGap, sanitizeReconcileSeed, isPlanningMetaPhase, phaseHasVerifiableAction } from '../longform/historical_guardrails.js'; // V0.91.3：阶段任务/少年权限硬防线 // V0.95.6：checkpoint 缺口本地愈合 // V0.102.17：标题不得冒充规划种子
import {
...
} from '../longform/longform_lifecycle.js'; // V0.92 全书阶段合同/收尾门
import {
