...
test('V0.25: 版本标识全局一致', () => {
  ...
  // V0.109.5：通用中文 AI 腔（抽象黑话/翻译腔/句式同质化/假升华）+ issue 类型语义注册表。
  assert.equal(pkg.version, '0.109.5');
  assert.equal(lock.version, '0.109.5');
  ...
  assert.ok(versionModule.includes("'0.109.5'"), '统一版本常量应为 0.109.5');
  assert.ok(web.includes('brand-ver">V0.109.5</span>'), '网页版本标识应为 V0.109.5');
  assert.match(pkg.description, /有界创作上下文.*原子叙事版本重建.*自进化/);
  assert.ok(pkg.description.includes('V0.109.5'), '包描述应同步当前版本');
