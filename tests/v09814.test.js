...
test('V0.98.14 对白跨句拆分偈语计入配额：3 处「不是X。…是Y。」→ medium（高赞"不是什么笑而是什么笑"形态）', async () => {
  const { detectAphorismQuota } = await import('../server/engine/quality/rules.js');
...
test('V0.98.14 叙述层跨句「不是X。…是Y。」点题对比：2 处升 medium，单字人称短否定豁免', async () => {
  const { detectNotButPattern } = await import('../server/engine/quality/rules.js');
...
test('V0.98.14 段首他她密度升档：≥45% 报 medium（参与审校修订），35-45% 保持 low 软信号', async () => {
  const { detectPronounParaDensity } = await import('../server/engine/quality/rules.js');
...
test('V0.98.14 起手式词表裂口补全：天还没亮/天未全亮 形态可命中（ch32 逐字重 ch28 必须本地可抓）', async () => {
  const { detectChapterOpenerTic } = await import('../server/engine/quality/rules.js');
...
test('V0.98.14 收束词表新增收怀贴物族：收进怀里贴着X 跨章复读 → medium（四章成族实证）', async () => {
  const { detectChapterEndingTic } = await import('../server/engine/quality/rules.js');
...
test('V0.98.14 母题词表扩充：指腹、蹲族变体（蹲得住/能蹲住/蹲住了）、小动作变体组可计数', async () => {
  const { detectMotifRepetition } = await import('../server/engine/quality/rules.js');
...
test('V0.98.14 章首句跨章逐字高权重：首句 ≥6 字与前章首句重合即报（12 字窗盲区补齐）', async () => {
  const { detectOpenerRepeat } = await import('../server/engine/quality/rules.js');
