...
describe('V0.78 新角色联动链路', () => {
  ...
  test('②looksLikePerson：老幺/小二/阿福等昵称识别为人物', async () => {
    const { looksLikePerson } = await import(pathToFileURL(path.join(ROOT, 'server/engine/narrative/pending.js')));
  ...
  test('⑥完整链路：正文【新设定:老幺】→ pending → tidy → 角色卡 → 点名册可见', async () => {
    const store = await import(pathToFileURL(path.join(ROOT, 'server/db/store.js')));
    const { stripNewSettingMarkers } = await import(pathToFileURL(path.join(ROOT, 'server/engine/pipeline/write.js')));
    const { tidyPendingEntities } = await import(pathToFileURL(path.join(ROOT, 'server/engine/narrative/pending.js')));
